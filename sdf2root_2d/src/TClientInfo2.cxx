...
ClassImp(TClientInfo2);

TClientInfo2::TClientInfo2()
    : fFile(0), fLocalName(), fContactsCount(0), fTimeSincePrevContact(0) {}

TClientInfo2::~TClientInfo2() {}
TClientInfo2::TClientInfo2(const char *filename, UInt_t clientId)
    : fFile(0), fContactsCount(0), fTimeSincePrevContact(0) {
  fLocalName.Form("%s-%d-%d", filename, clientId, gSystem->GetPid());
}

void TClientInfo2::SetFile(TFile *file) {
  {
...
void TClientInfo2::R__DeleteObject(TDirectory *dir, Bool_t withReset) {
  if (dir == 0)
    return;
...
void TClientInfo2::R__MigrateKey(TDirectory *destination, TDirectory *source) {
  if (destination == 0 || source == 0)
    return;
  TIter nextkey(source->GetListOfKeys());
  // std::cout<<"TClientInfo2::Trying to migrate the keys here"<<std::endl;
  TKey *key;
  while ((key = (TKey *)nextkey())) {
    TClass *cl = TClass::GetClass(key->GetClassName());
    if (cl->InheritsFrom(TDirectory::Class())) {
      ...
      TKey *newkey = new TKey(
          destination, *key,
          0 /* pidoffset */); // a priori the file are from the same client ..
